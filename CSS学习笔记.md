...
上述代码会将所有 <p> 段落元素的颜色设置为红色，字体大小设置为16px。
...
这里先将父元素设置为相对定位，然后子元素设置为绝对定位，子元素就会相对于父元素的左上角向左移动10px，向上移动10px，并且子元素原来在文档流中的空间不再保留。
...
这个 div 元素会固定在浏览器窗口的右上角，距离顶部10px，距离右侧10px，即使页面滚动也不会改变位置。
